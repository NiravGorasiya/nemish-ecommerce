...
const getAttributeCtrl = async (queryData) => {
    ...
    const attribute = await findModelItemsQ('Attributes', {
        offset,
        limit,
        order: [
            ['createdAt', 'DESC']
...
const updateAttributeCtrl = async (ctrlData) => {
    const attribute = await findModelItemQ('Attributes', { where: { id: ctrlData.attributeId } })
    if (attribute) {
        await attribute.update({ ...ctrlData })
        const attributeData = await findModelItemQ('Attributes', { where: { id: ctrlData.attributeId }, raw: true })
...
module.exports = {
    createAttributeCtrl, getAttributeCtrl, updateAttributeCtrl, deleteAttributeCtrl, getoneAttributeCtrl
}